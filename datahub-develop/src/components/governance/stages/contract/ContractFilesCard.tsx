// 계약 자료 카드 — 계약 단계 채팅에 첨부된 파일을 자동 수집해 리스트로 표시.
//
// 수집 기준: 채팅 단계맵(dh:gov:chat-stages)에서 stage === 계약(2) 인 메시지의 첨부만.
//   다른 단계(협의/진행 등) 첨부는 제외. 다운로드만 가능(삭제 불가).
// 협의 자료 카드(NegotiationFilesCard)와 동일 구조 — stage 인덱스만 다름.
...
/** 계약 단계 인덱스 (0=신청, 1=협의, 2=계약 …). */
export const CONTRACT_STAGE_INDEX = 2;
...
export function ContractFilesCard({ formId, refreshNonce }: Props) {
  ...
  const collect = useCallback(async () => {
    ...
    messages.forEach((m) => {
      ...
      if (stage !== CONTRACT_STAGE_INDEX) return;
  ...
  return (
    <section className="rounded-xl border-[0.5px] border-[var(--color-border-tertiary,#e5e7eb)] bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
      <header className="mb-3 flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-gray-400">계약 단계 채팅 첨부 자동 수집</span>
      </header>
