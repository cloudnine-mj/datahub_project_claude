// 2단계 / 단계 카드 wrapper — status 에 따라 3가지 디자인으로 분기 렌더.
//   done: 흰 배경 + solid 회색 테두리 + 녹색 체크 아이콘 + '완료' 배지 + 요약 박스 + '상세' 버튼
//   current: 흰 배경 + brand red 테두리 + 외부 글로우 + '진행 중' 배지 + body + '단계 완료' 버튼
//   locked: 회색 배경 + 점선 테두리 + opacity 0.7 + 자물쇠 아이콘 + 활성화 안내
...
function CurrentCard({
  ...
  return (
    <section
      ...
      id={id}
      aria-current="step"
      className="rounded-xl border border-red-200 bg-white px-5 py-4 shadow-[0_0_0_2px_rgba(220,38,38,0.06)] dark:border-red-900/40 dark:bg-gray-900"
    >
      <header className="flex items-center gap-2.5">
        <div className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-red-50 dark:bg-red-900/30">
          <Icon size={14} aria-hidden="true" className="text-red-700 dark:text-red-300" />
        </div>
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100">{title}</h3>
          <span className="inline-flex items-center gap-1 rounded-md bg-red-50 px-2 py-0.5 text-[10px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">
      ...
      <div className="mt-3.5 flex justify-end border-t border-gray-100 pt-3 dark:border-gray-800">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-md bg-red-50 px-3.5 py-1.5 text-xs font-medium text-red-700 transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-red-900/30 dark:text-red-300"
        >
          {completeLabel}
