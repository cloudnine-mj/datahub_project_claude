...
function adaptForm(f: Record<string, unknown>): FormDetail & Record<string, unknown> {
  const participants = Array.isArray((f.payload as Record<string, unknown> | null)?.["참조자"])
    ...
    : [];
  const camelHistory = (f.approvalHistory as { status: string; changedBy?: string; changedAt?: string; comment?: string | null; action?: string; actorRole?: string }[] | null) ?? null;
  // snake_case 호환 — datahub-web 옛 FormDetail/ApprovalEntry shape.
  // action / actorRole 은 진행 이력 타임라인이 이벤트 종류·주체를 정확히 분류하는 데
  // 쓰이므로 반드시 함께 전달 (누락 시 status 휴리스틱으로 잘못 분류됨).
  ...
  return {
    ...
    request_no: f.requestNo as string,
    form_type: f.formType as FormType,
    project_name: f.projectName as string,
    submitter_name: f.submitterName as string,
    submitter_email: f.submitterEmail as string,
    submitter_department: (f.submitterDepartment as string | null) ?? null,
    submitted_at: f.submittedAt as string,
    updated_at: f.updatedAt as string,
    parent_form_id: (f.parentFormId as string | null) ?? null,
    approval_history: snakeHistory as unknown as FormDetail["approval_history"],
    edit_history: (f.editHistory as FormDetail["edit_history"]) ?? null,
...
export const api = {
  ...
  appendFormEvent: async (
    id: string | number,
    body: { action: string; comment?: string; actorName?: string; actorRole?: string },
  ) =>
    adaptForm(
