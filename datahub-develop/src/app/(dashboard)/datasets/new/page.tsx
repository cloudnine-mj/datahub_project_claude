...
export default function NewDatasetPage() {
  ...
  const [publicAccess, setPublicAccess] = useState<PublicAccess>(
    presetForFineGrained(), // public 프리셋 — visibility=public 일 땐 백엔드가 무시
  );
  ...
  useEffect(() => {
    async function loadMeta() {
      try {
        const [lic, tsk, lng] = await Promise.all([
  ...
  const handleCreate = async () => {
    ...
    // Org info combined into group field as "project/techcell/org"
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <div className="rounded-xl border border-dh-border bg-white p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            id="description"
            rows={3}
            placeholder="데이터셋에 대한 간략한 설명을 입력하세요."
            value={description}
      ...
      <div className="rounded-xl border border-[#E9D5FF] bg-[#FAF5FF] p-6 space-y-4">
        ...
        <p className="text-xs text-text-secondary">
          AI가 초기 README.md (Dataset Card)와 메타데이터를 자동으로 생성합니다.
        </p>

        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-text-primary">Dataset Card 자동 생성</p>
