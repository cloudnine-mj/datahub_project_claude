...
def get_user_rbac_actions(
    ...
    Returns:
        ...
        set()      → 자원은 노출되나 user 가 가진 액션 없음. 호출 측에서 403
                     ...
                     - public + allow_anonymous_download=false + 비멤버
    ...
    repo: Optional[Repo] = (
        db.query(Repo).filter(Repo.repo_name == repo_name).first()
    )
    if repo is None:
        return None
    ...
    perm: Optional[Permission] = (
        db.query(Permission)
        .filter(Permission.repo_name == repo_name, Permission.user_id == user.id)
        .first()
    )
    if perm is not None:
    ...
    # 비멤버 — visibility + 다운로드 정책 따라 분기
    if repo.visibility == "private":
        # private 비멤버 → 자원 자체 미노출 → 404
        return None

    # public — 다운로드 허용 여부에 따라
    if repo.allow_anonymous_download:
        return {"read"}

    # public + 다운로드 거부 → 자원은 보이지만 액션 없음 → 403 insufficient permission
    return set()
