...
def make_org(db: Session, owner: User, name: str, visibility: str = "private") -> Organization:
    org = Organization(org_name=name, owner_id=owner.id, visibility=visibility)
    db.add(org)
    db.flush()
    db.refresh(org)
    org.owner = owner
...
def make_repo(db: Session, owner: User, org: Organization | None, name: str) -> Repo:
    repo = Repo(repo_name=name, owner_id=owner.id, org_id=org.id if org else None, visibility="private")
    db.add(repo)
    db.flush()
    return repo
