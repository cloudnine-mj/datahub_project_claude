...
def remove_bucket_lifecycle_rules(
    *,
    ...
    if not selected_buckets:
        effective_prefix = prefix or os.environ.get("GCP_BUCKET_PREFIX") or os.environ.get("DATAHUB_GCP_BUCKET_PREFIX")
        if not effective_prefix:
            raise ValueError(
                "bucket prefix is required when --bucket is not provided; "
                "set --prefix or GCP_BUCKET_PREFIX/DATAHUB_GCP_BUCKET_PREFIX"
            )
