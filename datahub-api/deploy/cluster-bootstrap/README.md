...
#    datahub.lgresearch.ai           → prd LB IP (web + /api/v1)
#    dev.datahub.lgair-data.com      → dev LB IP (web + /api/v1)
#    stg.datahub.lgair-data.com      → stg LB IP (web + /api/v1)
#
#    api.datahub.*, api-dev.datahub.*, api-stg.datahub.* 는 신규 API 진입점으로
#    만들지 않는다. 기존 값이 남은 클라이언트는 SDK/CLI endpoint 정규화로
#    단일 도메인 origin으로 이동시킨다.
```
