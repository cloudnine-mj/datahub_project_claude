...
class TestGetUserRbacActions:
    def _setup(self, *, owner_id=10, visibility="private", metadata_read=False):
        from app.models import Repo, RepoPublicAccessPolicy

        repo = Repo(
            repo_name="lab/r1",
            owner_id=owner_id,
            visibility=visibility,
            public_access_policy=RepoPublicAccessPolicy(
                repo_name="lab/r1",
                discoverable=metadata_read,
                metadata_read=metadata_read,
                file_list=metadata_read,
                file_read=metadata_read,
                lineage_read=metadata_read,
                stats_read=metadata_read,
            ),
        )
        db = MagicMock()
    ...
    def test_private_non_member_returns_none(self):
        ...
        """
        from app.models import Permission, Repo, RepoPublicAccessPolicy, User
    ...
    def test_public_policy_metadata_read_allows_non_member_read(self):
        from app.models import Repo, RepoPublicAccessPolicy, User
        ...
        perm_chain.filter.return_value.filter.return_value.first.return_value = None
        db = MagicMock()
        db.query.side_effect = lambda model: repo_chain if model is Repo else perm_chain

        actions = get_user_rbac_actions(db, User(id=10, email="alice@x"), "lab/r1")

        assert actions == {"read"}

    def test_policy_discoverable_without_metadata_returns_empty_set(self):
        from app.models import Repo, RepoPublicAccessPolicy, User
