...
2. RepoInfo — 신규 필드 포함 직렬화 (description, repo_type, member_count)
...
from app.schemas.repos import CreateRepoRequest, RepoInfo

# ── SQLite 테이블 선택 (JSONB 없는 것만) ───────────────────────────────────────

_SQLITE_TABLES = [
    Base.metadata.tables[t]
    for t in ("users", "repos", "permissions", "metadata_vocabularies")
]
...
class TestMetaEndpoints:
    ...
    def test_modalities_returns_db_seeded_list(self, db):
        _, client = _make_test_app(db)
        resp = client.get("/meta/modalities")
        assert resp.status_code == 200
        data = resp.json()
        assert any(item["id"] == "text" for item in data)

    def test_admin_can_create_and_deactivate_value(self, db, monkeypatch):
        from app.routers import meta as meta_mod

        monkeypatch.setattr(meta_mod.audit, "log", lambda *args, **kwargs: None)
        ...
        created = client.post(
            "/meta/tasks",
            json={"id": "custom-task", "name": "Custom Task", "sort_order": 100},
        )
        assert created.status_code == 200, created.text
        assert created.json()["id"] == "custom-task"
        listed = client.get("/meta/tasks")
        assert any(item["id"] == "custom-task" for item in listed.json())

        deleted = client.delete("/meta/tasks/custom-task")
        assert deleted.status_code == 200, deleted.text
        listed_after = client.get("/meta/tasks")
        assert all(item["id"] != "custom-task" for item in listed_after.json())

    def test_non_admin_cannot_create_value(self, db):
        _, client = _make_test_app(db)
        resp = client.post("/meta/tasks", json={"id": "blocked", "name": "Blocked"})
