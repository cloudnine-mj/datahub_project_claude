...
def test_org_membership_role_is_not_inherited_by_repo_access() -> None:
    db = _make_db()
    ...
    org = Organization(group_name="vision", owner_id=owner.id, visibility="private")
    db.add(org)
    db.flush()

    repo = Repo(repo_name="dataset-a", owner_id=owner.id, group_id=org.id, visibility="private")
    db.add(repo)
    db.flush()
    ...
    assert resolve_role(db, member, repo) is None
    assert _has_org_access(db, org, member) is True


def test_team_repo_permission_is_not_inherited_by_repo_access() -> None:
    db = _make_db()
    owner = User(email="owner@example.com", is_active=True)
    member = User(email="member@example.com", is_active=True)
    db.add_all([owner, member])
    ...
    team = Team(group_id=org.id, name="researchers", created_by=owner.id)
    db.add(team)
    db.flush()
...
def test_direct_repo_permission_still_wins_when_higher_than_org_or_team() -> None:
    ...
    db.add(OrganizationMembership(group_id=org.id, user_id=member.id, role="guest", granted_by=owner.id))
    team = Team(group_id=org.id, name="researchers", created_by=owner.id)
    db.add(team)
    db.flush()
    db.add(TeamMembership(team_id=team.id, user_id=member.id, added_by=owner.id))
    db.add(TeamRepoPermission(team_id=team.id, repo_name=repo.repo_name, role="contributor", granted_by=owner.id))
