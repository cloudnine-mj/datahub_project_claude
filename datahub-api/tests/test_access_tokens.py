...
class TestBearerRouting:
    ...
    def test_x_api_key_header_is_not_an_auth_path(self, monkeypatch):
        from app.dependencies import get_current_user

        def fake_authenticate(db, raw, request=None):
            raise AssertionError("X-API-Key must not call access token auth")
        ...
        request = MagicMock()
        request.headers.get.side_effect = lambda k: (
            "dh_aA8x3KpQ_test" if k.lower() == "x-api-key" else None
        )
        db = MagicMock()
