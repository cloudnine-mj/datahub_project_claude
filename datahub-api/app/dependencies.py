...
def _try_verify_azure_token(token: str) -> Optional[str]:
    """Azure AD id_token 검증, 이메일 반환. 실패 시 None.

    governance §api-specs/auth-api-spec §Azure AD OIDC. flag `AZURE_SSO_ENABLED`
    off 일 때는 None 반환 (transition 동안 dual run — Google + HS256 도 그대로).
    """
    if not settings.azure_sso_enabled:
        return None
    try:
        from app.services.azure_oauth import verify_azure_id_token, AzureTokenError
    except Exception:
        return None
    try:
        claims = verify_azure_id_token(token)
    except Exception:
        # AzureTokenError 또는 import-time 실패는 모두 silent None — Bearer 흐름의
        # 다음 분기 (Google) 로 폴백. 401 매핑은 호출 stack 상위 책임.
        return None
...
def get_current_user(request: Request, db: Session = Depends(get_db)) -> User:
    ...
    if auth_header:
        ...
        if len(parts) == 2 and parts[0].lower() == "bearer":
            ...
            # Azure AD id_token (flag on 일 때만 검증 시도)
            email = _try_verify_azure_token(token)
            if email:
                _set_user_auth_state(request, "azure_ad")
                return get_or_create_user(db, email)

            # Google access token (transition 동안 유지. PR 7 에서 제거)
    ...
    raise HTTPException(
        status_code=401,
        detail="Authorization header (Bearer token) required",
    )
...
def _extract_resource_id(request: Request, resource_type: str) -> Optional[str]:
    ...
    repo 식별자는 API 표면에서 `<group>/<repo>` (canonical) 이지만 DB 는
    repos.repo_name bare name + repos.group_id 로 namespace 를 표현한다
    (alembic 017 에서 `org_id` → `group_id` rename). 라우터 path 표기는
    세 종류로 공존한다:
      ...
      - canonical      `/repos/{owner}/{repo}`                   (CLI contract)
