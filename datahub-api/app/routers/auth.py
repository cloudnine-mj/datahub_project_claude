...
from app.services.azure_oauth import (
    AzureTokenError,
    build_authorize_url as _azure_build_authorize_url,
    exchange_code_for_id_token as _azure_exchange_code,
    verify_azure_id_token as _azure_verify,
)
...
def login(
    ...
    """OAuth 로그인 시작 — dual-IdP (Google + Microsoft).

    `provider`: 사용자 선택 IdP. governance §auth-and-api-keys.
      - "google" (기본): Google OAuth
      - "microsoft": Azure AD OIDC (AZURE_SSO_ENABLED 이 on 일 때만)
    ...
    if cli_paste:
        inner_state = f"paste:{cli_state}"
    elif cli_port:
        inner_state = f"cli:{cli_port}:{cli_state}"
    else:
        inner_state = redirect_uri or "/"

    provider_norm = (provider or "google").strip().lower()

    # Microsoft 분기 — flag on + state 에 ms: prefix 인코딩 (callback 이 provider 식별)
    if provider_norm in ("microsoft", "ms", "azure"):
        if not settings.azure_sso_enabled:
            raise HTTPException(
                status_code=503,
                detail="Azure SSO is not enabled in this environment",
            )
        state = f"ms:{inner_state}"
        try:
            url = _azure_build_authorize_url(state=state, redirect_uri=callback_url)
        except AzureTokenError as exc:
            raise HTTPException(status_code=500, detail=f"Azure SSO misconfigured: {exc}")
        return RedirectResponse(url)

    # Google (default) — state 그대로 (legacy 호환)
    state = inner_state
    params = {
...
def callback(
    ...
    # state 의 "ms:" prefix 가 Azure 흐름 표지. dual-IdP — Google 은 영구 유지.
    is_microsoft = state.startswith("ms:")
    if is_microsoft:
        state = state[len("ms:"):]   # inner state (paste:/device:/cli:/url) 추출

    if is_microsoft:
        try:
            id_token = _azure_exchange_code(code=code, redirect_uri=callback_url)
            claims = _azure_verify(id_token)
        except AzureTokenError as exc:
            raise HTTPException(status_code=401, detail=f"Azure SSO failed: {exc}")
        email = claims.get("email") or claims.get("preferred_username")
        if not email:
            raise HTTPException(status_code=401, detail="Azure token missing email claim")
        name = claims.get("name")
        # email 매칭으로 자동 linking (Google 사용자도 같은 row 사용)
        user = get_or_create_user(db, email, name=name)
        # azure_oid / tid / linked_at backfill + audit (PR 6 linking 강화)
        from app.services.azure_linking import link_azure_identity
        user = link_azure_identity(db, user, claims, audit=audit, request=request)
    else:
        # Authorization code → access token → user info (Google)
    ...
    # device flow 는 토큰을 polling 으로 받으므로 완료 페이지에 code 를 싣지
    # 않는다. /auth/complete 는 code 가 없으면 manual-paste fallback(빈 code
    # textarea) 을 렌더하므로, device flow 전용 success 페이지로 보낸다.
    if state.startswith("device:"):
        ...
        return RedirectResponse(
            f"{settings.frontend_url}/auth/device-success?"
...
def device_authorize(device_code: str, request: Request, provider: str = "google"):
    """브라우저 진입점 — device_code 검증 후 IdP 로 리디렉트.

    `provider`: 사용자 선택 IdP (governance §auth-and-api-keys, dual-IdP).
      - "google" (기본): Google OAuth
      - "microsoft": Azure AD OIDC (AZURE_SSO_ENABLED on 일 때만)
    ...
    callback_url = _get_oauth_callback_url(request)
    inner_state = f"device:{device_code}"
    provider_norm = (provider or "google").strip().lower()

    # Microsoft 분기 — state 에 ms: prefix
    if provider_norm in ("microsoft", "ms", "azure"):
        if not settings.azure_sso_enabled:
            raise HTTPException(
                status_code=503,
