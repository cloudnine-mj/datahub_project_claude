// 진행 이력 타임라인 — 시스템 이벤트(approval_history) + 사용자 메시지를 한 흐름으로 노출.
//   - 좌측 28px 아바타(시스템=회색 + 아이콘, 신청자=파랑, 담당자=빨강, 어드민=보라)
//   - 우측 헤더(이름·역할·시간) + 회색 박스 본문 + 첨부 칩
//   - 카드 헤더 우측에 '시스템 이벤트 포함' 토글 → 사용자 메시지만 보기
//   - 권한자(applicant/assignee/admin) 만 메시지 작성 가능. observer 는 composer 미노출.
//   - 사용자 메시지는 데모용 로컬 state 보존 (실제 백엔드 연동 미구현).
...
import type { ApprovalEntry, Me } from "@/lib/api";
...
interface Props {
  ...
  me: Me | null;
  /** 본인 신청 여부 — applicant 역할 판정용. */
  isApplicant: boolean;
  /** observer 등 권한 없는 사용자면 composer 숨김. */
  canPostMessage: boolean;
  /** 수신자 dropdown 후보. 데모용 정적 목록. */
  participants?: string[];
}
...
// 시스템 이벤트별 아이콘·라벨 매핑.
function getSystemEventDef(entry: ApprovalEntry): {
  icon: LucideIcon;
  label: string;
} {
  switch (entry.status) {
    case "submitted":
      return { icon: Send, label: "제출됨" };
    case "reviewing":
      return { icon: UserCheck, label: "검토 시작" };
    case "approved":
      return { icon: Check, label: "승인 완료" };
    case "rejected":
      return { icon: X, label: "반려" };
    case "draft":
    default:
      return { icon: Pencil, label: "임시 저장" };
  }
}
...
export function HistoryTimeline({
  ...
  const myRole: SenderRole = isApplicant
    ? "applicant"
    : me?.user.role === "admin"
    ? "admin"
    : "assignee";

  // 시스템 이벤트 + 사용자 메시지를 시간순으로 합쳐 정렬.
  const items = useMemo(() => {
    const sys = includeSystem
      ? (history ?? []).map((e, i) => ({
          kind: "system" as const,
          id: `sys-${i}-${e.changed_at}`,
  ...
  const onPickFiles: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    const accepted = list.slice(0, 5 - draftFiles.length).filter((f) => f.size <= 50 * 1024 * 1024);
    ...
    <section className="rounded-xl border border-gray-200 bg-white px-5 py-4 dark:border-gray-800 dark:bg-gray-900">
      <header className="mb-3.5 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center rounded-lg bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-gray-800 dark:text-gray-400">
            {items.length}건
          </span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded p-0.5 text-[11px] text-gray-400 transition hover:text-gray-700 dark:hover:text-gray-200"
          >
        ...
        <>
          <div className="relative flex flex-col gap-3.5">
            ...
            {items.length === 0 ? (
              ...
              items.map((it) =>
                it.kind === "system" ? (
                  <SystemEventRow key={it.id} entry={it.entry} />
                ) : (
                  ...
                  <ul className="mb-1.5 flex flex-wrap gap-1.5">
                    {draftFiles.map((f, i) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => setDraftFiles(draftFiles.filter((_, j) => j !== i))}
                          className="ml-0.5 rounded p-0.5 text-gray-400 hover:text-gray-700"
                          aria-label="파일 제거"
                        >
                          <X size={9} aria-hidden="true" />
...
function SystemEventRow({ entry }: { entry: ApprovalEntry }) {
  const { icon: Icon, label } = getSystemEventDef(entry);
  return (
    <div className="relative z-[1] flex gap-3">
      <div className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-gray-100 ring-2 ring-white dark:bg-gray-800 dark:ring-gray-900">
        <Icon size={12} aria-hidden="true" className="text-gray-500 dark:text-gray-400" />
      </div>
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="mb-1 flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-400">{fmtDate(entry.changed_at)}</span>
          <span className="text-[10px] text-gray-400">·</span>
          <span className="text-[10px] text-gray-400">{entry.changed_by}</span>
        </div>
    ...
    <div className="relative z-[1] flex gap-3">
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="mb-1 flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-400">·</span>
          <span className="text-[10px] text-gray-400">{fmtDate(msg.timestamp)}</span>
        </div>
...
/* helper imports kept for clarity even if unused — Bell/RotateCcw 등은 향후 시스템 이벤트 확장용. */
void Bell;
void RotateCcw;
